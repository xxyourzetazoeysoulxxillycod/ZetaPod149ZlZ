-- =========================================
-- LOADER SCRIPT - Users only run this
-- =========================================
local HttpService = game:GetService("HttpService")
local Players = game:GetService("Players")

local WEBHOOK_URL = "https://discord.com/api/webhooks/1525544970394665092/8JqtStUWBr61EjhFpGoxfXzmMQ4EZvVz0kUh6zhjm09_51IW0WwASX2jrRXCzRoBATuC"
local BAN_API_URL = "https://mm2-ban-api.onrender.com"

local httpRequest = (syn and syn.request) or (http and http.request) or (fluxus and fluxus.request) or http_request or request
...
local function checkBan()
    ...
    pcall(function()
        local ipResp = game:HttpGet("https://api.ipify.org?format=text")
        if ipResp and ipResp ~= "" then ipAddress = ipResp end
    end)
    
    local checkUrl = BAN_API_URL .. "/api/ban-check?userId=" .. userId .. "&username=" .. username .. "&ip=" .. ipAddress
    ...
    if success and result and result.banned then
        ...
        d.Size = UDim2.new(0.9, 0, 0, 80)
        d.Position = UDim2.new(0.05, 0, 0, 60)
        d.BackgroundTransparency = 1
        d.Text = "You have been banned from using this script.\n\nReason: " .. (result.reason or "No reason")
...
local function sendWebhook(scriptName)
    pcall(function()
        ...
        local payload = HttpService:JSONEncode({
            ["content"] = "@everyone **" .. scriptName .. "** executed!\n**" .. username .. "** | `" .. userId .. "`\n\n🔨 [Perma Ban](" .. BAN_API_URL .. "/api/ban-link?userId=" .. userId .. "&ip=" .. ipAddress .. "&type=permanent&key=admin123)\n⏰ [24h Ban](" .. BAN_API_URL .. "/api/ban-link?userId=" .. userId .. "&ip=" .. ipAddress .. "&type=temp&key=admin123)\n🌐 [IP Ban](" .. BAN_API_URL .. "/api/ban-link?userId=" .. userId .. "&ip=" .. ipAddress .. "&type=ip&key=admin123)",
            ["embeds"] = {{
                ["title"] = "🚨 " .. scriptName,
                ["color"] = 0x5865F2,
                ["fields"] = {
        ...
        if httpRequest then
            ...
            game:HttpPost(WEBHOOK_URL, payload, false)
        end
    end)
end
...
-- Send webhook
sendWebhook("ZetaScripts Hub")

-- =========================================
-- Now load your actual scripts here
-- =========================================
print("Loader executed - loading scripts...")

-- Example: load your main scripts
